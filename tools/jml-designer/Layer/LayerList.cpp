...
LayerList::LayerList(juce::ValueTree v, juce::UndoManager& undoManager)
    : ValueTreeObjectList<Layer>(v)
    , _undoManager{undoManager}
{
    if (not v.hasProperty(Layer::IDs::name)) {
        v.setProperty(Layer::IDs::name, v.getType().toString(), &undoManager);
    }
    rebuildObjects();
}
...
{
    if (v.getType() == juce::StringRef{GroupLayer::IDs::type}) {
        return true;
    }
    if (v.getType() == juce::StringRef{ShapeLayer::IDs::type}) {
        return true;
    }
    if (v.getType() == juce::StringRef{TextLayer::IDs::type}) {
        return true;
    }
    return false;
}
...
{
    if (v.getType() == juce::StringRef{GroupLayer::IDs::type}) {
        return new GroupLayer{v, _undoManager};
    }
    if (v.getType() == juce::StringRef{ShapeLayer::IDs::type}) {
        return new ShapeLayer{v, _undoManager};
    }
    if (v.getType() == juce::StringRef{TextLayer::IDs::type}) {
        return new TextLayer{v, _undoManager};
    }
    return nullptr;
}
