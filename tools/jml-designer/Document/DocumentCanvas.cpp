...
DocumentCanvas::DocumentCanvas(Document& doc, ToolBar& toolBar)
    : _document{doc}
    , _tool{std::make_unique<SelectionTool>(*this)}
{
...
auto DocumentCanvas::updateTool(ToolType type) -> void
{
    ...
    if (type == ToolType::rectangle) {
        _tool = std::make_unique<ShapeTool>(*this, Shape::Drawable);
    }
    if (type == ToolType::ellipse) {
        _tool = std::make_unique<ShapeTool>(*this, Shape::Ellipse);
    }
    repaint();
}
} // namespace jml::designer
