...
function jml.VerticalLayout(spec)
  function spec.perform(area, children)
    local heights = calculatePercentageBasedSize(area:getHeight(), children,
                                                 "height")
    for k in pairs(children) do
...
function jml.HorizontalLayout(spec)
  function spec.perform(area, children)
    local widths = calculatePercentageBasedSize(area:getWidth(), children,
                                                "width")
    for k in pairs(children) do
      assert(widths[k] ~= nil)
      local child = children[k]
...
function jml.Slider(spec)
  ...
  function spec.build()
    ...
    if spec["range"] ~= nil then
      slider:setRange(spec.range["start"], spec.range["stop"],
                      spec.range["interval"])
    end
