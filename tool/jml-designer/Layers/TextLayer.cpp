...
{
    if (not tree.hasProperty(property)) {
        tree.setProperty(property, value, um);
    }
}
} // namespace
...
auto TextLayer::setText(juce::String const& text) -> void
{
    valueTree().setProperty(IDs::text, text, undoManager());
}
...
auto TextLayer::setFontSize(float fontSize) -> void
{
    valueTree().setProperty(IDs::fontSize, fontSize, undoManager());
}
...
auto TextLayer::addLayerProperties(juce::PropertyPanel& panel) -> void
{
    auto const properties = juce::Array<juce::PropertyComponent*>{
        ...
        makeJustificationProperty(
            valueTree().getPropertyAsValue(IDs::justification, undoManager()),
            "Justification"
        )
            .release(),
    };
